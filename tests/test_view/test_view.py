...
def test_catpk_to_name():
    ...
    cats = [Category("cat1", pk=1),
            Category("cat2", pk=2),]
    view.create_categories(cats)
    assert view._category_pk2name(1) == "cat1"
    assert view._category_pk2name(3) == ""
...
def test_set_expenses():
    ...
    view.create_expenses(exps)
    assert view.expenses == exps
    assert view.expenses_table.expense_list == exps
...
def test_set_budgets():
    ...
    bdgs = [Budget(1000, "day", spent=100),
            Budget(7000, "week"),]
    view.create_budgets(bdgs)
    assert view.budgets == bdgs
    assert view.budget_table.budget_list == bdgs
